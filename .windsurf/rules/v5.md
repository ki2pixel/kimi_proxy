...
- **Listing**: Utilisez `fast_list_directory(path="/home/kidpixel/kimi-proxy/memory-bank")` pour lister les fichiers disponibles.
- **Project Listing**: Utilisez `fast_get_directory_tree(path="/home/kidpixel/kimi-proxy/memory-bank")` pour diagnostiquer la connectivité en cas d'échec d'accès.
...
- **Authorized Tools**: `fast_edit_block` (via API proxy), `fast_read_multiple_files` (via API proxy), outils filesystem-agent et ripgrep-agent (MCP intégrés Continue).
- **Note importante**: Les outils MCP Phase 4 (task-master, sequential-thinking, fast-filesystem, json-query) **ne sont pas disponibles** directement dans Continue.dev. Utiliser les endpoints API du proxy Kimi (`/api/memory/*`).
