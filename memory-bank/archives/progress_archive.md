...
### [2026-02-25 09:30:00] - MCP — Implémentation du masquage des observations et pruner API — TERMINÉ
**Statut** : ✅ COMPLETÉ
**Description** : Réduction majeure de la consommation de tokens entrante (15% - 40%) et sécurisation contre les plantages du parseur de l'IDE face à des retours MCP massifs (ex: listings de gros dossiers).

**Implémentation** :
1. Masquage au vol `role=tool` (Schéma 1) dans `PassthroughProcessor`. Seul le nom de l'outil et sa présence (masqué) sont visibles par le LLM.
2. Endpoint API de contexte pruné `/api/mcp-gateway/pruner/` implémentant le masquage pour les requêtes MCP manuelles (serveur proxy autonome "pruner").
3. Conservation de la trace (prune_id et lignes modifiées) et fallback no-op transparent.

**Fichiers ajoutés / modifiés** :
- `src/kimi_proxy/proxy/passthrough.py`
- `src/kimi_proxy/api/routes/mcp_gateway.py`

**Validation** :
- Un appel à `fast_list_directory` / `fast_search_files` depuis Cline remonte ~250 caractères vs 30,000+.
- Logs transparents et temps de traitement `~2ms`.
