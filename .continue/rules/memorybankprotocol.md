...
7. **Locking Instruction**: Use absolute paths exclusively for memory-bank files in `/home/kidpixel/kimi-proxy/memory-bank/`. Use EXCLUSIVELY the fast-filesystem and filesystem-agent MCP tools. Do NOT attempt to read memory-bank files via regular filesystem tools (read_file).
...
-  - **Archiving Tool**: Use `fast_write_file(path="/home/kidpixel/kimi-proxy/memory-bank/archives/...")` for archiving old entries. Never delete data without archiving via this tool first.
